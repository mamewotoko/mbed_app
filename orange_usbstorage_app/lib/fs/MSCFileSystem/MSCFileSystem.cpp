...
int MSCFileSystem::initialise_msc()
{
    ...
    rc = MS_Init( (USB_INT32U*)&_blkSize, (USB_INT32U*)&_numBlks, inquiryResult );
    if (rc != OK)
    {
...
int MSCFileSystem::disk_write(const uint8_t * buffer, uint64_t block_number)
{
    if ( OK == MS_BulkSend((USB_INT32U)block_number, 1, (USB_INT08U *)buffer) )
        return 0;
    return 1;
}

int MSCFileSystem::disk_read(uint8_t * buffer, uint64_t block_number)
{
  if ( OK == MS_BulkRecv((USB_INT32U)block_number, 1, (USB_INT08U *)buffer) )
        return 0;
    return 1;
}
...
uint64_t MSCFileSystem::disk_sectors() { return (uint64_t)_numBlks; }
